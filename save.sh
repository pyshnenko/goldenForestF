...
then
  ...
  while true; do
    read -r -p "do you need to create pages folder? (Y) or (N) " answer
    case $answer in
        [Yy]* ) 
            while IFS= read -r line
            do
                if [[ "$line" != "" ]]; then
                    mkdir /var/www/html/$1/$line
                    cp $1/index.html /var/www/html/$1/$line
                    echo "$line created"
                fi
            done < folders.txt
            break;;
  ...
  while true; do
    read -r -p "do you need to move json and fonts? (Y) or (N) " answer
    case $answer in
        [Yy]* ) cp toSave/* /var/www/html/$1; break;;
        [Nn]* ) break;;
        * ) echo "Please answer Y or N.";;
    esac
  done 
  ...
  while true; do
    read -r -p "do you need to update access on nginx folder? (Y) or (N) " answer
    case $answer in
        [Yy]* ) chmod 777 -R /var/www/html; break;;
        [Nn]* ) break;;
        * ) echo "Please answer Y or N.";;
    esac
  done 
